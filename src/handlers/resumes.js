...
async function uploadUrl(event) {
  try {
    const { userId } = await requireAuthWithClaims(event, { requireTamuEmail: false });
 ...
 * Behavior: HeadObject S3, confirm PDF, update DynamoDB, update StudentProfiles.resumeS3Key
 */
async function complete(event) {
  try {
    ...
    if (profile) {
      await studentProfilesService.update(userId, { resumeS3Key: record.s3Key });
    }
